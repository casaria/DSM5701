...
{
  { PA_PRINT, "WFS-VS", 2, 2},
  { PA_SCROLL_UP, "LEFT: ", 2, 1 },
  {  PA_SCROLL_DOWN_LEFT, C1, 2, 3},
  { PA_SCROLL_DOWN, "RIGHT: ", 2, 1},
  { PA_SCROLL_DOWN_LEFT, C2, 2, 3},
...
\
  ...
  if (topicStr.endsWith("DSM5701")) {
    ...
    newColourString = "Cooler A: 2.4°C \nCooler B: 4.2°C";
...
void setup() {
  ...
  */
  ...
  xTaskCreate (updateScreen, "Screen", 4096, NULL, 2, NULL);
...
void updateScreen (void *pvParameters) {
  ...
  for (;;) {
    if (xSemaphoreTake (serialMutex, (TickType_t)10) == pdTRUE) {
      ...
      tft.drawString("88:88:88",110,25, 7);
      tft.setTextColor(rgb565Decimal, TFT_BLACK);
      tft.drawString (timeString, 110, 25, 7);
      tft.drawString (colourString, 10, 69, 3);
    }
    esp_task_wdt_reset();
    vTaskDelay (1000 / portTICK_PERIOD_MS);
  }
